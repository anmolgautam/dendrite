"""Alembic env.py — async migration runner for Dendrite.

Supports both SQLite (default) and Postgres (via DENDRITE_DATABASE_URL).
...
# (e.g. via `dendrite db migrate --url postgres://...`)
...
if not existing_url or existing_url == "sqlite+aiosqlite:///./dendrite.db":
